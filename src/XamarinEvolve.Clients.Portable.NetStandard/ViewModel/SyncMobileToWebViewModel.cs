...
namespace XamarinEvolve.Clients.Portable
{
	public class SyncMobileToWebViewModel : ViewModelBase
	{
        ...
        protected override void UpdateCommandCanExecute()
        {
            ...
            getSyncCodeCommand?.ChangeCanExecute();
        }
		...
		Command getSyncCodeCommand;
		public ICommand GetSyncCodeCommand =>
            getSyncCodeCommand ?? (getSyncCodeCommand = new Command(() => ExecuteGetSyncCodeAsync().IgnoreResult(ShowError)));

		public bool GetSyncCodeButtonAvailable => !IsBusy && !ResultVisible;

		Command doneCommand;
		public ICommand DoneCommand =>
            doneCommand ?? (doneCommand = new Command(() => Navigation.PopAsync(true).IgnoreResult(ShowError)));
