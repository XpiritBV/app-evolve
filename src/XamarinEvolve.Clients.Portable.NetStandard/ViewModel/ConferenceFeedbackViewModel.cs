...
namespace XamarinEvolve.Clients.Portable
{
	public class ConferenceFeedbackViewModel : ViewModelBase
	{
        ...
        {
            submitFeedbackCommand?.ChangeCanExecute();
        }
		...
		public ICommand SubmitFeedbackCommand =>
            submitFeedbackCommand ?? (submitFeedbackCommand = new Command(() => ExecuteSubmitFeedbackCommandAsync().IgnoreResult(ShowError), () => !IsBusy));
		...
		async Task ExecuteSubmitFeedbackCommandAsync()
		{
			...
			try
			{
                ...
                var versionProvider = Locator.Get<IAppVersionProvider>();
				...
				var feedback = new ConferenceFeedback
				{
					...
					DeviceOS = Device.RuntimePlatform.ToString(),
