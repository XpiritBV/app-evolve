...
namespace XamarinEvolve.Clients.Portable
{
	public static class ReminderService
	{
		...
		{
			var ready = await CheckPermissionsGetCalendarAsync().ConfigureAwait(false);
			if (!ready)
			{
				return false;
			}
			...
			try
			{
				var evolveCal = await GetOrCreateEvolveCalendarAsync().ConfigureAwait(false);
				var externalId = Settings.Current.GetEventId(id);
				var calEvent = await CrossCalendars.Current.GetEventByIdAsync(externalId).ConfigureAwait(false);
				await CrossCalendars.Current.DeleteEventAsync(evolveCal, calEvent).ConfigureAwait(false);
				Settings.Current.RemoveReminderId(id);
				Settings.Current.HasSetReminder = true;
			}
			catch (Exception ex)
						...
						if (Device.RuntimePlatform == Device.iOS)
						{
							MessagingService.Current.SendMessage<MessagingServiceQuestion>(MessageKeys.Question, new MessagingServiceQuestion
							{
								...
								OnCompleted = (result) =>
								{
									if (result)
									{
										GlobalDependencyService.Locator.Get<IPushNotifications>().OpenSettings();
									}
								}
							});
						}
