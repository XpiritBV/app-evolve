    ...
    class EventColorDisplayConverter : IValueConverter
    {
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            try
            {
                ...
                return Clock.Now > ((DateTime)value).ToUniversalTime() ? Color.FromHex("D3D2D2") : (Color)Application.Current.Resources["Primary"];
            }
            catch (Exception ex)
            {
                Debug.WriteLine("Unable to convert: " + ex);
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            try
            {
                if (value is DateTime) 
                {
                    ...
                    {
						return date.Humanize (utcDate: false, culture: EventInfo.Culture);
                    }

					return date.Humanize(culture: EventInfo.Culture);
                }
            }
            catch (Exception ex)
            {
