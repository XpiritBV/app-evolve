	...
	public partial class TodayViewController : UIViewController, INCWidgetProviding
	{
		...
		bool IsInitialized()
		{
			try
			{
				...
				var initialized = settings.BoolForKey("FavoritesInitialized");
		...
		{
            Console.WriteLine($"{AboutThisApp.PackageName} - WidgetPerformUpdate");
			...
			if (IsInitialized())
			{
				try
				{
					...
					{
						MainTitleLabel.Hidden = false;
						MainTitleLabel.Text = "You have no upcoming favorites any time soon (code 1)";
						SessionsTable.Hidden = true;
					}
			...
			{
				MainTitleLabel.Hidden = false;
				MainTitleLabel.Text = "You have no upcoming favorites any time soon (code 2)";
				SessionsTable.Hidden = true;
                completionHandler(NCUpdateResult.NewData);
			}
		}
